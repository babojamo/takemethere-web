...
export function FloatingRouteList({
  route_fares,
  onRouteClick,
  top = 16,
  right = 16,
        ...
        <Accordion activeIndex={activeIndex} onTabChange={onTabChange}>
          {route_fares.map((r, i) => (
            ...
            <AccordionTab key={`route-${i}`} header={`Route ${i + 1}`}>
              <p><strong>Total:</strong> {r.total_fare}</p>
              <p><strong>Breakdown:</strong></p>
              <ul>
                {r.route_fare.map((rf, j) => (
                  <li key={`r${j}`}><span style={{ color: rf.route.points_color ?? 'black' }}> {rf.route.name}</span>: {rf.estimate_fare}</li>
                ))}
              </ul>
            </AccordionTab>
